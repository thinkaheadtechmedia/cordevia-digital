...
export const AboutView: React.FC<AboutViewProps> = ({ onNavigate, onOpenAvatarModal, onShowToast }) => {
  ...
  const [showAboutPassInput, setShowAboutPassInput] = React.useState(false);
  const [aboutPassword, setAboutPassword] = React.useState('');
  const [aboutPassError, setAboutPassError] = React.useState(false);

  const handleAboutPasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (loginAsAdmin(aboutPassword)) {
      onShowToast?.('Admin Access Granted', 'Official brand identity suite unlocked.', 'success');
      setShowAboutPassInput(false);
      setAboutPassword('');
      setAboutPassError(false);
    } else {
      setAboutPassError(true);
      onShowToast?.('Verification Failed', 'Invalid administrator password. Please try again.', 'warning');
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-20">
      ...
      {/* Official Brand Identity & Social Avatar Kit Showcase (Admin Only) */}
      {isAdmin && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
            ...
            <div className="lg:col-span-4 flex flex-col items-center text-center">
              ...
              <span className="text-xs text-slate-400 mt-3 font-mono">
                Official Cordevia Digital Monogram CD (Admin Asset)
              </span>
            </div>
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-800/60 text-cyan-400 text-xs font-semibold">
                <Sparkles className="w-3.5 h-3.5" />
                <span>Official Brand Kit & Social Asset Suite • Admin Access</span>
              </div>
